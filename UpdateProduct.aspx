        ...
        <tr align="left">
            ...
            <td align="left">
                <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" DataSourceID="SqlDataSource3"
                    HorizontalAlign="Left" DataKeyNames="productid" Height="245px" Width="100%">
                    <Columns>
                        <asp:ImageField DataImageUrlField="productimage" DataImageUrlFormatString="uploads/{0}"
                            HeaderText="Picture">
                            <ControlStyle Height="100px" Width="75px" />
                        </asp:ImageField>
                        <asp:BoundField DataField="productname" HeaderText="Product " SortExpression="productname" />
                        <asp:BoundField DataField="rate" HeaderText="Rate" SortExpression="rate" />
                        <asp:CommandField DeleteImageUrl="~/imgs/trash_(delete)_16x16.gif" HeaderText="Delete"
                            ShowDeleteButton="True">
                            <ControlStyle Height="20px" Width="25px" />
                        </asp:CommandField>
                        <asp:HyperLinkField DataNavigateUrlFields="productid" DataNavigateUrlFormatString="updateproductdetails.aspx?pid={0}"
        ...
        <tr>
            ...
            <td>
                <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConflictDetection="CompareAllValues"
                    ConnectionString="<%$ ConnectionStrings:MyConnectionString %>" DeleteCommand="DELETE FROM [addproduct] WHERE [productid] = @original_productid AND (([productname] = @original_productname) OR ([productname] IS NULL AND @original_productname IS NULL))  AND (([rate] = @original_rate) OR ([rate] IS NULL AND @original_rate IS NULL)) AND (([description] = @original_description) OR ([description] IS NULL AND @original_description IS NULL)) AND (([productimage] = @original_productimage) OR ([productimage] IS NULL AND @original_productimage IS NULL))"
                    InsertCommand="INSERT INTO [addproduct] ([productname], [rate], [description], [productimage]) VALUES (@productname, @rate, @description, @productimage)"
                    OldValuesParameterFormatString="original_{0}" SelectCommand="SELECT [productid], [productname], [rate], [description], [productimage] FROM [addproduct] WHERE (([catid] = @catid) AND ([subcatid] = @subcatid))"
                    UpdateCommand="UPDATE [addproduct] SET [productname] = @productname, [rate] = @rate, [description] = @description, [productimage] = @productimage WHERE [productid] = @original_productid AND (([productname] = @original_productname) OR ([productname] IS NULL AND @original_productname IS NULL)) OR ([brandid] IS NULL AND @original_brandid IS NULL)) AND (([rate] = @original_rate) OR ([rate] IS NULL AND @original_rate IS NULL)) AND (([description] = @original_description) OR ([description] IS NULL AND @original_description IS NULL)) AND (([productimage] = @original_productimage) OR ([productimage] IS NULL AND @original_productimage IS NULL))">
                    <DeleteParameters>
                    ...
                    <SelectParameters>
                        <asp:ControlParameter ControlID="DropDownList1" Name="catid" PropertyName="SelectedValue"
                            Type="Decimal" />
                        <asp:ControlParameter ControlID="DropDownList2" Name="subcatid" PropertyName="SelectedValue"
                            Type="Decimal" />
                    </SelectParameters>
                    <UpdateParameters>
                        <asp:Parameter Name="productname" Type="String" />
